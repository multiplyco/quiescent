...
public abstract class AbstractBoundedChannel implements IChannel, IBuffered {
    ...
    // Final producer sequence captured at seal time. Used to allow consumers
    // to detect claims beyond the last producible slot and bail out.
    protected volatile long finalProducerSeq;
    ...
    protected AbstractBoundedChannel(int requestedSize, boolean padded) {
        ...
        // Until sealed, treat as unbounded.
        this.finalProducerSeq = Long.MAX_VALUE;
    }
    ...
    protected boolean park(Object[] registry, int pIdx, long expectedAvail, long prevStep,
        ...
        while (true) {
            ...
            // If we're a consumer (expectedAvail >= 0) and the channel is sealed,
            // and we've claimed a slot at/after the final producer sequence, bail.
            if (expectedAvail >= 0 && (long) PRODUCER_SEQ.getVolatile(this) < 0) {
                int idx = pIdx >> ashft; // physical index for this slot
                long slotCandidate = (expectedAvail << sizeShift) | (long) idx;
                if (slotCandidate >= finalProducerSeq) return false;
            }
    ...
    public boolean seal() {
        ...
        // Capture the terminal producer sequence so consumers can detect
        // claims beyond the last producible slot.
        finalProducerSeq = prev;
