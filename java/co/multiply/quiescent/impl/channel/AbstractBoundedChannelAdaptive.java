...
/**
 * Abstract base for adaptive bounded channels. Owns the full adaptive
 * take implementation and the put-side infrastructure (lock, condition,
 * buffer). Subclasses implement {@link #put(Object)}:
 * <ul>
 *   <li>{@link BoundedChannelAdaptive} — adaptive owner-tracking with
 *       Dekker handshake for lock-free single-producer fast path.</li>
 *   <li>{@link BoundedChannelAdaptiveXf} — transducer support; acquires
 *       {@code putLock} directly and calls {@link #putDirect(Object)}.</li>
 * </ul>
 * <p>
 * The consumer side uses the same adaptive ownership protocol in both
 * variants: single-consumer fast path, irreversible upgrade to locked.
 */
public abstract class AbstractBoundedChannelAdaptive implements IChannel, IBuffered {
    ...
    static {
        try {
            COUNT = MethodHandles.lookup().findVarHandle(
                AbstractBoundedChannelAdaptive.class, "count", int.class);
    ...
    AbstractBoundedChannelAdaptive(int requestedSize) {
        if (requestedSize < 1)
